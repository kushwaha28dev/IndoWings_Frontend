      ...
      <div className="max-w-[1240px] mx-auto px-4 sm:px-6">
        <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,0.92fr)_minmax(320px,1fr)] gap-10 lg:gap-12 items-center">
          ...
          <div>
            ...
            <p className="text-base sm:text-lg text-zinc-200/75 leading-relaxed mb-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <div className="p-4 sm:p-5 rounded-lg border border-white/15 bg-white/[0.06] text-white/80 flex flex-col justify-between min-h-[95px]">
                <HardDriveDownload className="w-5 h-5 text-zinc-400 mb-2.5" />
                <div>
                  <strong className="block text-white text-lg sm:text-xl font-bold leading-tight">Windows</strong>
                  <span className="text-xs text-zinc-300/70">MSI installer</span>
                </div>
              </div>

              {/* Role-aware */}
              <div className="p-4 sm:p-5 rounded-lg border border-white/15 bg-white/[0.06] text-white/80 flex flex-col justify-between min-h-[95px]">
                <KeyRound className="w-5 h-5 text-zinc-400 mb-2.5" />
                <div>
                  <strong className="block text-white text-lg sm:text-xl font-bold leading-tight">Role-aware</strong>
                  <div className="flex items-center gap-1.5 text-xs text-zinc-300/70 mt-0.5">
                    <span>tools</span>
                    <span 
                      className="relative inline-flex items-center justify-center w-4 h-4 rounded-full bg-zinc-100 text-zinc-900 text-[10px] font-black cursor-help group"
                    >
                      ?
                      <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block w-56 p-2.5 bg-[#09090b] text-white text-xs font-normal rounded-lg shadow-xl border border-white/10 z-50 text-left pointer-events-none">
              ...
              {/* Synced data */}
              <div className="p-4 sm:p-5 rounded-lg border border-white/15 bg-white/[0.06] text-white/80 flex flex-col justify-between min-h-[95px]">
                <DatabaseZap className="w-5 h-5 text-zinc-400 mb-2.5" />
                <div>
